...
Bomberman::Explosion::Explosion(int x, int y) : _irr(Bomberman::Irrlicht::instance()) {
  ...
  this->_em = _ps->createBoxEmitter(irr::core::aabbox3d<irr::f32>(-7,0,-7,7,1,7), irr::core::vector3df(0.0f,0.0f,0.0f), 10, 50, irr::video::SColor(0,255,255,255), \
				    irr::video::SColor(0,255,255,255), 800, 2000, 180, irr::core::dimension2df(1.f,1.f), irr::core::dimension2df(2.f,2.f));
  _ps->setEmitter(this->_em);
  _em->drop();
  _paf = _ps->createFadeOutParticleAffector();
  _ps->addAffector(_paf); // same goes for the affector
  _paf->drop();
